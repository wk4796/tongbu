...
## 🚨 5. 紧急救援：硬盘已满清理指南
如果你因为使用了挂载路径导致硬盘爆满（100%），即使脚本停止了，空间可能依然没有释放。这是因为 Rclone 的后台缓存还残留着未上传完的数据。
### 现象
* 脚本报错 `Input/output error`。
* 运行 `df -h` 发现 `/` 根目录占用率 100%。
* 删除了 `/tmp/tongbu_cache` 后空间依然不够。
...
**3. 清理 Rclone 系统缓存 (关键步骤)**
这是导致“空间双重占用”的罪魁祸首。位于 `/root/.cache/rclone/` 或 `~/.cache/rclone/`。
**注意**：这里存放的是“正在上传但未完成”的文件碎片。既然已经报错失败，这些碎片已无价值，直接删除即可。
确认路径存在
```
ls -lh /root/.cache/rclone/
```
执行强制删除
```
